...
def score_experiment_models(experiment_definition_name, rows, model_names):
    experiment_name = experiment_definition_name + str(rows)
    ...
    predictions = score_trained_models(data_file_path, model_names, classification_col, experiment_dir, prefix=experiment_name)
    return predictions
...
def score_experiments_models(exp_dict, model_names):
    ...
    for exp_definition in experiments:
        ...
        for rows_per_it in iterations:
            predictions[exp_definition][rows_per_it] = score_experiment_models(exp_definition, rows_per_it, model_names)
